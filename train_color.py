...
from utils import to_psnr, to_ssim_skimage, to_ciede2000, validation, print_train_log, print_test_log, print_train_log_color, print_test_log_color
import os
import lr_scheduler
import math
...
def ColourDistance(rgb_1, rgb_2):
    rgb_1 = rgb_1.reshape(3,-1).to(device)
    rgb_2 = rgb_2.reshape(3,-1).to(device)
    R_1,G_1,B_1 = torch.mean(rgb_1[0]), torch.mean(rgb_1[1]), torch.mean(rgb_1[2]) 
    R_2,G_2,B_2 = torch.mean(rgb_2[0]), torch.mean(rgb_2[1]), torch.mean(rgb_2[2])
    rmean = (R_1 +R_2 ) / 2
    R = R_1 - R_2
    G = G_1 -G_2
    B = B_1 - B_2
...
for epoch in range(1, opt.nEpochs + 1):
    ...
    for iteration, inputs in enumerate(train_dataloader,1):
        ...
        loss_G = criterionL1(fake_B, gt) + 0.1 * ColourDistance(fake_B, gt)
        loss_G.backward()
        optimizer_G.step()
    ...
    save_checkpoints = './checkpoints/color/'
    if os.path.isdir(save_checkpoints)== False:
        os.mkdir(save_checkpoints)

    print_train_log_color(epoch, train_epoch, train_psnr, train_ssim, train_ciede2000, 'T-Cloud', (time.time()-epoch_start_time)/60)

    if epoch % 5 == 0:
        netG.eval()
        test_psnr, test_ssim, test_ciede2000 = validation(netG, test_dataloader, device)
        print_test_log_color(epoch, train_epoch, test_psnr, test_ssim, test_ciede2000, 'T-Cloud')

        # --- Save the network  --- #
        torch.save(netG.state_dict(), './checkpoints/color/cloud_removal_epoch_' + str(epoch) + '.pth')
...
torch.save(netG.state_dict(), './checkpoints/color/cloud_removal_epoch_latest.pth')
